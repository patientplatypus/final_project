
<template>
  <div class="my-drawing">
    <div v-if="canDrawOK === true">
      <div v-if="drawOrInject === 'draw'">
        <canvas id="canvas" v-on:mousedown="handleMouseDown" v-on:mouseup="handleMouseUp" v-on:mousemove="handleMouseMove" width="800px" height="800px"></canvas>
      </div>
      <div v-if="drawOrInject === 'inject'">
        <canvas id="canvas" v-on:mousedown="handleInject" width="800px" height="800px"></canvas>
      </div>
    </div>
    <div v-else-if="canDrawOK === false">
      <div class="shownotahotdog">
        <h1>Here is the picture everyone made!</h1>
      </div>

        <!-- <div id="imgcontainer" width="400px" height="400px">
          <p>hello there sailor</p>

          <p>hello there sailor</p>
        </div> -->

        <!-- <div v-html='canvasImageData'></div> -->

        <!-- <canvas id="canvas" width="800px" height="800px" style="zoom:50%"></canvas> -->

        <!-- <div class="imgcontainer" v-html='canvasImageData.outerHTML' style="max-width:400px; max-height:400px;"></div> -->

        <div class="imgcontainer" style="max-height: 400px; max-width:400px;">
          <img id="canvasImage">
        </div>



         <!-- <canvas id="canvas" width="800px" height="800px" style="zoom:50%"></canvas> -->



      <div class="shownotahotdog">
        <h1>So the question you got to ask yourself punk....</h1>
        <h2>...ishotdog? or isnothotdog?</h2>
          <button class="button" v-on:click='ishotdog'>ishotdog</button>
          <button class="button" v-on:click='isnothotdog'>isnothotdog</button>
        <h3>Number who say ishotdog {{this.$parent.ishotdog}}</h3>
        <h3>Number who say isnothotdog {{this.$parent.isnothotdog}}</h3>
        <h3>Therefore percent hotdog is {{percenthotdog()}}</h3>
      </div>


    </div>
    <div v-else-if="canDrawOK === null">
      <div class="rules">
        <h2>Here are the rules of the game</h2>
        <h2>Once someone presses the hotdogtimer button on the left
        everyone will have 2 minutes to draw notahotdog. The goal of the game
        is to draw whatever you want, as long as it's NOT a hotdog.</h2>
        <h3>example</h3>
        <div class="imgleft">
          <img src="http://i.imgur.com/eaYO3PB.jpg" alt="ishotdog" height="200" width="200">
          <p> this ishotdog </p>
        </div>
        <div class="imgright">
          <img src="http://i.imgur.com/uE6XxXB.jpg" alt="ishotdog" height="200" width="200">
          <p> this isnothotdog (isplatypus)</p>
        </div>
        <h2> Of course there might be one or two total jabronis that will try and draw a hotdog just to mess with everyone's head! Afterwards everyone gets to vote on whether it's a hotdog or not, and has the option of playing again.
        Have fun!</h2>
      </div>
    </div>
  </div>
</template>


<style scoped>
  .my-drawing {
    /*display: inline-block;*/
    /*float: left;*/
    display:flex;
    /*width: 50%;*/
    text-align: center;
    align-items:center;
    justify-content:center;
    background-color: #0D2149;
    margin-top: 5px;
    height: 800px;
  }
  .imgcontainer img{
    max-width: 400px;
    max-height: 400px;
  }
  .imgleft{
    margin-right: 10%;
    display: inline-block;
    text-align: center;
  }
  .imgright{
    margin-left: 10%;
    display: inline-block;
    text-align: center;
  }
  .imgcontainer{
    background-color: #F9EBE0;
    margin: 0 auto;
  }
  .rules {
    color: white;
    text-align: center;
    padding-left: 10%;
    padding-right: 10%;
  }
  .shownotahotdog{
    color: white;
    /*height: 783px;*/
    text-align: center;
    padding-left: 10%;
    padding-right: 10%;
  }
  .button{
    color: black;
  }
</style>

<script>
export default {
  data: function () {
    return {
      mouse: {
        current: {
          x: 0,
          y: 0
        },
        previous: {
          x: 0,
          y: 0
        },
        down: false
      },
      canvasImage:[],
      colorToUse:"rgba(0,0,0,1)",
      sizeToUse: 2,
      canDrawOK: null,
      canVote: true,
      drawOrInject: "draw",
      canvasImageData: "pants"
    }
  },
  props: {
    useThisColor: {
      type: String
    },
    useThisCanvas: {
      type: Array
    },
    useThisSize: {
      type: String
    },
    canDraw: {
      type: Boolean
    }
  },
  // directives:{
  //   saveCanvas: function(canvasElement, nding){
  //      this.canvasImageData = canvasElement.toDataURL('image/jpeg', 1.0);
  //   }
  // },
  watch: {
    canDraw (n,o){
      console.log('canDraw watch ', n,o);

      this.canDrawOK=n

      // console.log("right after canvasImageData in canDraw and value of this.canvasImageData is, ", this.canvasImageData);
      // console.log('right after canvasImageData in canDraw and value is, ', canvasImageData);


      if (n === true){
        var c = document.getElementById('canvas');
        var ctx = c.getContext('2d');
        ctx.clearRect(0,0,800,800);
      }
      if (n === false){

        // console.log('n equals false and this.canvasImageData is ', this.$parent.canvasImageData);

          // console.log('right inside n===false and canvasImage is', canvasImageData);


        // var c = document.getElementById('imgcontainer');
        // console.log("this is the imgcontainer element data ", c);
        // c.append(this.canvasImageData);

        // console.log("inside n false for canDrawOK");
        //
        // var cSmaller=document.getElementById("canvasSmaller");
        // var ctxSmaller=c.getContext("2d");
        //
        // ctxSmaller.drawImage(c, 0, 0, 800,  800,     // source rectangle
        //                    0, 0, 400, 400);

        // var c2=document.getElementById("canvasSmaller");
        // var ctx2=c2.getContext("2d");

        // ctx.scale(0.5, 0.5);
        // ctx.drawImage(c, 400, 400);

        // var canvas = document.getElementById('canvas');
        // var ctx = canvas.getContext('2d');
        // var img = new Image;
        // img.onload = function(){
        //   ctx.drawImage(img,0,0); // Or at whatever offset you like
        // };
        // img.src = this.canvasImageData;
      }
    },
    canDrawOK(n,o){
      if (n===false){

        setTimeout(function(){
          console.log("canDrawOk is false!!!!");

          var img = document.getElementById('canvasImage');
          console.log('value of img after getElementById', img);
          // console.log('****************** LOCALSTORAGE canvasImageData ******************', localStorage.getItem('canvasImageData'));
          img.setAttribute('src', localStorage.getItem('canvasImageData'));
          localStorage.clear();
        }, 1000);



      }
    },
    useThisSize (n,o){
      // console.log('useThisSize watch, ', n,o);
      this.sizeToUse=n;
    },
    canvasImage (n,o){
      // console.log("canvasImage watch, ", n[0], o[0]);
    },
    useThisColor (n, o) {
      // console.log("useThisColor watch, ", n, o); // n is the new value, o is the old value.
      // console.log("first letter of useThisColor", n.charAt(0));

      if (n.charAt(0)==="h"){
        this.drawOrInject="inject";
        this.colorToUse=n;
      }
      if(n.charAt(0)==="r"){
        this.drawOrInject="draw";
        this.colorToUse=n;
      }

    },
    useThisCanvas (newCanvas, oldCanvas) {
        // console.log("useThisCanvas watch, ", newCanvas[0], oldCanvas[0]) // n is the new value, o is the old value.

        // console.log('top of useThisCanvas and this is ', this);

        var canvasImageData = "";


        if (newCanvas[0][3]!=-1){
          var looplength = newCanvas.length;

          var canvasID = Date.now();
          window["createCanvas" + canvasID] = document.createElement('canvas');
          window["createCanvas" + canvasID].width = 800;
          window["createCanvas" + canvasID].height = 800;
          window["createCanvas" + canvasID].background = "transparent";
          window["createCanvas" + canvasID].borderRadius = "3px";
          window["createCanvas" + canvasID].borderColor = "black";
          window["createCanvas" + canvasID].borderStyle = "solid";

          window["ctx" + canvasID] = window["createCanvas" + canvasID].getContext('2d');

          if (looplength>1){

            for (var x = 1; x<looplength; x++){
              window["ctx" + canvasID].beginPath();
              window["ctx" + canvasID].moveTo(newCanvas[x-1][0], newCanvas[x-1][1]);
              window["ctx" + canvasID].lineTo(newCanvas[x][0], newCanvas[x][1]);
              window["ctx" + canvasID].strokeStyle = newCanvas[x][2];
              window["ctx" + canvasID].lineWidth = newCanvas[x][3];
              window["ctx" + canvasID].stroke();
              window["ctx" + canvasID].closePath();
            }
          }

          var canvas = document.getElementById('canvas');
          var ctx = canvas.getContext('2d');
          ctx.drawImage(window["createCanvas" + canvasID], 0, 0);

          canvasImageData = canvas.toDataURL('image/png', 1.0);
          localStorage.setItem('canvasImageData', canvasImageData);

          window["createCanvas" + canvasID].remove();

          // canvas.setAttribute('crossOrigin', 'anonymous');
          // this.canvasImageData = canvas.toDataURL('image/jpeg', 1.0);

          // var img = new Image;
          // img.crossOrigin = "Anonymous";
          // img.src= canvas.toDataURL('image/jpeg', 1.0);
          //
          // this.canvasImageData = img;
          // console.log("this.canvasImageData is ", this.canvasImageData);

          // console.log("right before image set and this.$parent is ", this);


          // console.log("this.canvasImageData is ", this.$parent.canvasImageData);



          // img.onload = function(){
          //   // img.style.width = '50%';
          //   // img.style.height = 'auto';
          //   this.canvasImageData = img;
          //   console.log("this.canvasImageData is ", this.canvasImageData);
          // }


      }else{

        console.log("inside the else statement");
        console.log('this is the value of the newCanvas object ', newCanvas.length);

        var canvasID = Date.now();
        window["createCanvas" + canvasID] = document.createElement('canvas');
        window["createCanvas" + canvasID].width = 800;
        window["createCanvas" + canvasID].height = 800;
        window["createCanvas" + canvasID].background = "transparent";
        window["createCanvas" + canvasID].borderRadius = "3px";
        window["createCanvas" + canvasID].borderColor = "black";
        window["createCanvas" + canvasID].borderStyle = "solid";

        window["ctx" + canvasID] = window["createCanvas" + canvasID].getContext('2d');

        window["imageObj" + canvasID] = new Image();
        window["imageObj" + canvasID].crossOrigin = "Anonymous";
        window["imageObj" + canvasID].src = newCanvas[0][2];

        window["imageObj" + canvasID].onload = function(){
          window["ctx" + canvasID].drawImage(window["imageObj" + canvasID], newCanvas[0][0], newCanvas[0][1]);
          var canvas = document.getElementById('canvas');
          var ctx = canvas.getContext('2d');
          ctx.drawImage(window["createCanvas" + canvasID], 0, 0);

          canvasImageData = canvas.toDataURL('image/png', 1.0);
          localStorage.setItem('canvasImageData', canvasImageData);
          // ctx.drawImage(window["createCanvas" + canvasID], 0, 0);
            // ctx.drawImage('https://i.stack.imgur.com/TMCfE.png', 500,500);
          window["createCanvas" + canvasID].remove();
          // this.drawOrInject = "draw";
          // this.colorToUse = 'r'
          //https://i.stack.imgur.com/TMCfE.png

          // canvas.setAttribute('crossOrigin', 'anonymous');
          // this.canvasImageData = canvas.toDataURL('image/jpeg', 1.0);
          // var img = new Image;
          // img.crossOrigin = "Anonymous";
          //
          // img.onload = function() {
          //   ctx.drawImage( img, 100, 100 );
          //   var imgTag = document.getElementById('myImg');
          //   var dataURL = ctx.toDataURL();
          //   imgTag.src = dataURL;
          // }
          // img.src = src;
          //
          // var myCanvas = document.getElementById('my_canvas_id');
          // var ctx = myCanvas.getContext('2d');
          // var img = new Image;
          // img.crossOrigin = "Anonymous";
          // img.onload = function(){
          //   ctx.drawImage(img,0,0); // Or at whatever offset you like
          // };
          // img.src = strDataURI;

          // var img = new Image;
          // img.crossOrigin = "Anonymous";
          // img.src= canvas.toDataURL('image/jpeg', 1.0);
          //
          // this.canvasImageData = img;
          // console.log("this.canvasImageData is ", this.canvasImageData);

          // this.canvasImageData = canvas.toDataURL('image/jpeg', 1.0);
          // console.log("this.canvasImageData is ", this.canvasImageData);

          // console.log("right before image set and this.$parent is ", this);
          //
          // this.$parent.canvasImageData = canvas.toDataURL('image/jpeg', 1.0);
          // console.log("this.canvasImageData is ", this.$parent.canvasImageData);




          // img.onload = function(){
          //   // img.style.width = '50%';
          //   // img.style.height = 'auto';
          //   this.canvasImageData = img;
          //   console.log("this.canvasImageData is ", this.canvasImageData);
          // }



        }



      }
      // console.log('1) bottom of useThisCanvas and this.canvasImageData is', this.canvasImageData);
      // console.log('2) bottom of useThisCanvas and canvasImageData before set is ', canvasImageData);
      // this.canvasImageData = canvasImageData;
      // console.log("3) bottom of useThisCanvas and this.canvasImageData after set is ", this.canvasImageData);
    }
  },
  computed: {
    currentMouse: function () {
      var c = document.getElementById("canvas");
      var rect = c.getBoundingClientRect();

      return {
        x: this.mouse.current.x - rect.left,
        y: this.mouse.current.y - rect.top
      }
    },
    previousMouse: function () {
      var c = document.getElementById("canvas");
      var rect = c.getBoundingClientRect();

      return {
        x: this.mouse.previous.x - rect.left,
        y: this.mouse.previous.y - rect.top
      }
    }
  },
  methods: {
    draw: function (event) {
     if (this.mouse.down) {
       var c = document.getElementById("canvas");
       var ctx = c.getContext("2d");
      //  ctx.clearRect(0,0,800,800);

       ctx.lineTo(this.currentMouse.x, this.currentMouse.y);
       console.log('right before strokeStyle (where you set color) and this.sizeToUse is ', this.sizeToUse);
       ctx.strokeStyle = this.colorToUse;
       ctx.lineWidth = this.sizeToUse;
       ctx.stroke();
       var dataArray = [this.currentMouse.x, this.currentMouse.y, this.colorToUse, this.sizeToUse];
       this.$set(this.canvasImage, 'myCanvas', this.canvasImage.push(dataArray));
     }
    },
    handleMouseDown: function (event) {
      this.mouse.current = {
        x: event.pageX,
        y: event.pageY
      }
      var c = document.getElementById("canvas");
      var ctx = c.getContext("2d");
      ctx.beginPath();
      ctx.moveTo(this.currentMouse.x, this.currentMouse.y);
      // if (this.mouse.down===false){
      //   ctx.beginPath();
      // }
      this.mouse.down = true;
    },
    handleMouseUp: function (event) {
      this.mouse.down = false;
      // var c = document.getElementById("canvas");
      // var ctx = c.getContext("2d");
      // ctx.closePath();
      this.$parent.channel2.push('new_img', {body:this.canvasImage});
      var looplength = this.canvasImage.length;
      for(var x=0; x<=looplength; x++){
        this.canvasImage = this.canvasImage.splice(x,1);
      }
      console.log("after for loop in handleMouseUp and new empy this.canvasImage has length", this.canvasImage.length);

    },
    handleMouseMove: function (event) {
      this.mouse.current = {
        x: event.pageX,
        y: event.pageY
      }
      // if (this.useThisColor.charAt(0)==="r"){
        this.draw(event)
      // }
    },
    ishotdog: function(){
      if (this.canVote){
        this.$parent.channel4.push('hotdog_counter', {body:"ishotdog"});
        this.canVote = false;
      }
    },
    isnothotdog: function(){
      if (this.canVote){
        this.$parent.channel4.push('hotdog_counter', {body:"isnothotdog"});
        this.canVote = false;
      }
    },
    percenthotdog: function(){
      if (this.$parent.ishotdog===0 && this.$parent.ishotdog===0){
        return "_____"
      }else{
        return Math.round(100*this.$parent.ishotdog/(this.$parent.ishotdog+this.$parent.isnothotdog))
      }
    },
    handleInject: function(){

      // this.mouse.current = {
      //   x: event.pageX,
      //   y: event.pageY
      // }

      var c = document.getElementById("canvas");
      var ctx = c.getContext("2d");
      var rect = c.getBoundingClientRect();

      var useX = event.pageX - rect.left;
      var useY = event.pageY - rect.top;

      var imageObj = new Image();
      imageObj.src = this.colorToUse;

      console.log("you've clicke at (x,y) ", useX, ", ", useY);

      imageObj.onload = function() {
        ctx.drawImage(imageObj, useX-50, useY-50);
      };

      var dataArray = [useX-50, useY-50, this.colorToUse, -1];

      var looplength = this.canvasImage.length;
      for(var x=0; x<=looplength; x++){
        this.canvasImage = this.canvasImage.splice(x,1);
      }

      this.$set(this.canvasImage, 'myCanvas', this.canvasImage.push(dataArray));
      this.$parent.channel2.push('new_img', {body:this.canvasImage});

      var looplength = this.canvasImage.length;
      for(var x=0; x<=looplength; x++){
        this.canvasImage = this.canvasImage.splice(x,1);
      }

    }
  },
  // clearfunction: function(){
  //     console.log("INSIDE CLEARFUNCTION *****************");
  //     var c = document.getElementById('canvas');
  //     var ctx = c.getContext('2d');
  //     ctx.clearRect(0,0,800,800);
  //     return true
  // },
  ready: function () {
    var c = document.getElementById("canvas");
    var ctx = c.getContext("2d");
    ctx.translate(0.5, 0.5);
    ctx.imageSmoothingEnabled= false;

  }
}
</script>
